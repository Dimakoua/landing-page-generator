    ...
    const pageInfo = await page.evaluate(() => {
      ...
      document.querySelectorAll("*").forEach((el) => {
        ...
        if (
          ...
          bgColorCounts[style.backgroundColor] =
            (bgColorCounts[style.backgroundColor] || 0) + 1;
        }
      });
      ...
      const h1Style = getComputedStyle(
      ...
      return {
        ...
        theme: {
          colors: {
            primary:
              allVars["--color-primary"] ||
              allVars["--primary"] ||
              allVars["color-primary"] ||
              allVars["primary"] ||
              getDominant(bgColorCounts)[0],
            secondary:
              allVars["--color-secondary"] ||
              allVars["--secondary"] ||
              allVars["color-secondary"] ||
              allVars["secondary"] ||
              getDominant(bgColorCounts)[1],
    ...
    const sections = await page.evaluate((baseUrl) => {
      ...
      const simplifyHtml = (el) => {
        ...
        clone.querySelectorAll("svg").forEach((svg) => {
          ...
          if (svg.innerHTML.length > 500)
            svg.innerHTML = '<path d="...truncated..."/>';
        });
        // Remove comments
      ...
      const isSection = (el) => {
        ...
        if (el.tagName === "DIV") {
          ...
          if (el.querySelector("section, header, footer, nav, main, article"))
            return false;
        ...
        if (isSection(el)) {
          ...
          if (style.backgroundImage !== "none") {
            ...
            for (const m of matches) {
              if (m[1])
                images.push({ src: resolveUrl(m[1]), type: "background" });
            }
          }
          ...
          if (
            htmlStr.includes("swiper") ||
            htmlStr.includes("slick") ||
            htmlStr.includes("slider") ||
            htmlStr.includes("carousel")
          ) {
          ...
          results.push({
            ...
            originalTitle:
              heading?.innerText.trim() || `Section ${results.length}`,
...
app.post("/api/dev/analyze", async (req, res) => {
  ...
  try {
    const result = await limiter.schedule(() =>
      gemini.generateJSON(
        ANALYZE_SYSTEM_PROMPT,
        getAnalyzeUserPrompt(title, theme, sections),
      ),
    );

    // Safety check for critical keys
    if (!result.mappings || !Array.isArray(result.mappings)) {
      result.mappings = [];
    }

    if (!result.theme) {
      result.theme = { colors: theme?.colors || {}, fonts: theme?.fonts || {} };
    }
    ...
    // Final consistency check: Ensure we didn't miss any critical sections (e.g., if AI skipped them)
    // We only re-attach if they were found, but we could also inject defaults here if needed.
...
app.post("/api/dev/generate", async (req, res) => {
  ...
  if (existingSessionId && sessions.has(existingSessionId)) {
    ...
    const tasks = [
      ...
      {
        id: "configs",
        name: "Generate JSON configurations",
        status: "pending",
      },
    ];
    mappings.forEach((m) => {
      if (m.isNew)
        tasks.push({
          id: `comp-${m.sectionId}`,
          name: `Generate component for ${m.sectionId}`,
          status: "pending",
        });
    });
  ...
  try {
    ...
    if (getTask("configs").status !== "done") {
      ...
      session.fileBuffer.set(
        path.join(landingPath, "theme.json"),
      ...
      session.fileBuffer.set(
        path.join(landingPath, "flow.json"),
      ...
      const layout = {
        sections: mappings.map((m) => {
          ...
          const componentName = m.isNew
            ? `Auto${m.suggestedComponent}${pascalId}`
            : m.suggestedComponent;
          return {
            id: m.sectionId,
      ...
      session.fileBuffer.set(
        path.join(stepsPath, "desktop.json"),
        JSON.stringify(layout, null, 2),
      );
      session.fileBuffer.set(
        path.join(stepsPath, "mobile.json"),
        JSON.stringify(layout, null, 2),
      );
    ...
    for (const m of mappings) {
      if (m.isNew) {
        ...
        try {
          const codeResponse = await limiter.schedule(() =>
            gemini.generateJSON(
              GENERATE_COMPONENT_SYSTEM_PROMPT,
              getGenerateComponentUserPrompt(
                componentName,
          ...
          session.fileBuffer.set(
            componentPath,
            sanitizeReactCode(codeResponse.code),
          );
          ...
          throw new Error(
            `AI generation failed for ${componentName}: ${err.message}`,
          );
        }
      }
    }
    ...
    console.log(
      `[Wizard] Session ${session.id} is READY. Waiting for redirect before flush...`,
    );
